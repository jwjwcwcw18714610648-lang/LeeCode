...
//若父节点值仍等于新的 inorder[inorderIndex]，说明父节点的左子树（即 p 所在的整棵左子树）也空了，继续弹……
    ...
    public int quickselect(int[] nums,  int l, int r,int k) {
      ...
      int i=l-1;
      int j=r+1;
      while(i<j){// 把区间分成 <x 和 >x 的两部分，i 与 j 相向扫描
          do i++;while(nums[i]<x);
          do j--;while(nums[j]>x);
          if(i<j){
              //说明两个指针相遇不了 即出现逆序 交换两指针值 最后i==j
              int tmp = nums[i];
              nums[i] = nums[j];
              nums[j] = tmp;
          }
    ...
    public int[] topKFrequent(int[] nums, int k) {
        Map<Integer,Integer> map=new HashMap<>();
        for (int num : nums) {
            map.put(num,map.getOrDefault(num,0)+1);//填充次数表
        }
        PriorityQueue<int[]> queue=new PriorityQueue<>(
                Comparator.comparingInt(a->a[1]));//小项堆
       for (Map.Entry<Integer, Integer> entry : map.entrySet()) {
            int num   = entry.getKey();
            int count = entry.getValue();

            if (queue.size() == k) {          // 堆已满
                if (queue.peek()[1] < count) { // 新元素比“当前最小候选”更大
                    queue.poll();              // 把最小的踢出去
    ...
    public void qsort(List<int[]> values, int start, int end, int[] ret, int retIndex, int k) {
        ...
        Collections.swap(values,start,index);//将基准放到中间（之前处于start位置 作为分割线）
        ...
        if (k<leftSize){
            ...
            if(k>=leftSize){
                qsort(values, index + 1, end, ret, retIndex, k - leftSize);
            }
        }
    }
